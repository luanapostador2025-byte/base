...
-- Set outfit id (the outfit id needs to exist on the client). Returns false if the outfit id is 0, nil if hud doesn't exists anymore.
...
-- Set hud zIndex. Higher zIndex will be drawn on top of lower zIndex.
-- Can be used only if newFeatures is enabled
function HUD:setZIndex(zIndex)
    if self.destroyed then return end
    if not self.newFeatures then return end

    hudSetZIndex(self.id, zIndex)
end

-- Set hud phantom state. A phantom hud will not block mouse events to huds or other components below it. You can use it to create click-through huds.
-- Can be used only if newFeatures is enabled
function HUD:setPhantom(phantom)
    if self.destroyed then return end
    if not self.newFeatures then return end

    hudSetPhantom(self.id, phantom)
end
